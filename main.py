...
def edit_entry(line_in_handbook:int):
    if line_in_handbook > len(entries):
...
def search_entries():
    ...
    display_entry(result) if result else print('Таких записей нет(((')
...
choice = int(input('Ввыберите действие'))
...
match choice:
    ...
    case 2:
        ...
        edit_entry(2) # Для редактирования записи нужно передать номер записи (Обратите внимание на количество записей)
    case 3:
        display_entry(entries)  # Функция на отображение постранично
    case 4:
